...
if events.empty?
  ...
  events.each do |event|
    ...
    next if event.attendees && event.attendees.select {|attendee| attendee.email.start_with?(ENV['BB_LOGIN']) && %w(accepted tentative).include?(attendee.response_status)}.empty?
